...
| 9 | `รหัสหลักประกันธนาคารแห่งประเทศไทย` | `bot_coll_code` | ✅ | `รหัส - คำอธิบาย` | `286003 - ที่ดิน` |
| 12 | `ราคาประเมินรวม` | `aprs_value` | ⬜ | ตัวเลข (ว่างไว้ = สุ่ม 10k–9.9M) | `500,000.00` |
| 14 | `AccType` | — | ⬜ | สำหรับ Type 5 bond เท่านั้น | `(1) ผู้กู้ 0-4 ปี` |
| 16 | `วันหมดอายุสัญญาเงินกู้` | `custom_enddate_collateral_date` | ⬜ | `DD/MM/YYYY` พ.ศ. | `14/12/2572` |
| **22** | **`coll_subtype`** | **`coll_subtype`** | ⬜ | ตัวเลข subtype (ว่างไว้ = ใช้ค่า default) | `1` |
| **23** | **`รหัสหลักประกันย่อย`** | **`sub_bot_coll_code`** | ⬜ | ตัวเลข (ว่างไว้ = ใช้ค่า default) | `0` |

> ✅ = ต้องกรอก / ⬜ = optional (ว่างไว้ script จะใช้ค่า default ตาม collateral type)
>
> 🆕 **Col 22–23** คือ override columns — ถ้ากรอกไว้จะ override ค่าที่ script generate ให้อัตโนมัติ
...
```
Seq.,Create Datatest Date,ID-Branch,Branch,Customer Type (ผู้กู้),Grade,CIF,ID-COLL,TYPE-COLL,รหัสหลักประกันธนาคารแห่งประเทศไทย,ContractNo,หมดอายุสัญญา,ราคาประเมินรวม ,DepAcc ,AccType,Conditions,วันหมดอายุสัญญาเงินกู้,สาขา/หน่วย,กลุ่ม,สถานะบัญชี,Used,หมายเหตุ,coll_subtype,รหัสหลักประกันย่อย
